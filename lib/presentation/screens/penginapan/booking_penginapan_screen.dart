...
class _BookingPenginapanScreenState
    extends State<BookingPenginapanScreen> {
  ...
  Future<void> _loadData() async {
    // ✅ Load dari API via WisataRepository → /wisata/{id}/penginapan
    ...
    if (mounted) {
      setState(() {
        if (data != null && data.isNotEmpty) {
          ...
          // Fallback dummy jika API masih kosong
          final dummyKamar = [
            const KamarModel(
              id: 101, tipeKamar: 'Deluxe Room Nature View',
              hargaPerMalam: 350000, kapasitas: 2,
              totalKamar: 4, tersedia: true,
            ),
            const KamarModel(
              id: 102, tipeKamar: 'Executive Family Suite',
              hargaPerMalam: 650000, kapasitas: 4,
              totalKamar: 2, tersedia: true,
            ),
          ];
          _penginapanList = [
            PenginapanModel(
              id: 1, nama: 'Citiis Nature Lodge & Resort',
              kamar: dummyKamar,
            ),
          ];
  ...
  Future<void> _booking() async {
    ...
    // ✅ Tambah notifikasi lokal
    context.read<NotifikasiProvider>().tambahNotifikasi(
      judul: 'Booking Penginapan Berhasil! 🏠',
      pesan :
          '${_selectedKamar!.tipeKamar} selama $_durasi malam senilai ${CurrencyFormatter.format(_total)} berhasil dibuat.',
      tipe: 'sukses',
    );

    final String bookingId = (r['success'] == true)
        ? (r['booking_id'] ?? 'BGP-${DateTime.now().millisecondsSinceEpoch}')
        : 'BGP-${DateTime.now().millisecondsSinceEpoch}';
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => PembayaranScreen(
          ...
          totalHarga  : _total,
          layanan     : _selectedKamar?.tipeKamar ?? 'Booking Penginapan',
        ),
      ),
    );
  }
            ...
            child: _loadingData
                ...
                : SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildSectionLabel('Pilih Penginapan'),
                        const SizedBox(height: 12),
                        ..._buildPenginapanSelector(),
                        const SizedBox(height: 20),
                        _buildSectionLabel('Pilih Tipe Kamar'),
                        const SizedBox(height: 12),
                        ..._buildKamarList(),
                        const SizedBox(height: 20),
                        _buildDateSection(),
  ...
  List<Widget> _buildPenginapanSelector() => _penginapanList.map((p) {
        ...
        return GestureDetector(
          onTap: () => setState(() {
            ...
            _selectedKamar =
                p.kamar.isNotEmpty ? p.kamar.first : null;
          }),
          child: Container(
  ...
  List<Widget> _buildKamarList() =>
      (_selectedPenginapan?.kamar ?? []).map((k) {
  ...
  Widget _buildDateSection() => Container(
        ...
        child: Row(
          children: [
            Expanded(
                child: _dateTile('Check-In', _checkin,
                    () => _pilihTanggal(true))),
            Container(
                width: 1, height: 40,
            ...
            Expanded(
                child: _dateTile('Check-Out', _checkout,
                    () => _pilihTanggal(false))),
          ],
        ),
      );
          ...
          children: [
            ...
            Row(
              children: [
                ...
                _counterBtn(
                  Icons.add_rounded,
                  _jumlahTamu < (_selectedKamar?.kapasitas ?? 4),
                  () => setState(() => _jumlahTamu++),
                ),
              ],
            ),
  ...
  Widget _counterBtn(IconData ico, bool enabled, VoidCallback onTap) =>
      GestureDetector(
        ...
        child: Container(
          width: 34, height: 34,
          decoration: BoxDecoration(
            color: enabled ? AppColors.info : AppColors.background,
